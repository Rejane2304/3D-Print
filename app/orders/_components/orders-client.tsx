...
const statusConfig: Record<string, { icon: React.ElementType; color: string; label: string }> = {
  pending: { icon: Clock, color: "text-amber", label: "Pendiente" },
  paid: { icon: CheckCircle, color: "text-green-400", label: "Pagado" },
  shipped: { icon: Package, color: "text-cyan", label: "Enviado" },
...
export function OrdersClient() {
  ...
  useEffect(() => {
    ...
    if (status === "unauthenticated") {
      ...
      // Los administradores no pueden tener pedidos propios, redirigir a panel admin
  ...
  return (
    <div className="min-h-screen py-8">
      <div className="max-w-site mx-auto px-4">
        <h1 className="text-3xl font-bold mb-2 flex items-center gap-3"><Package className="w-7 h-7 text-cyan" /> Mis Pedidos</h1>
        <p className="text-zinc-400 text-sm mb-8">{orders?.length ?? 0} pedido{(orders?.length ?? 0) !== 1 ? "s" : ""}</p>

        {(orders?.length ?? 0) === 0 ? (
          <div className="text-center py-20">
            <CreditCard className="w-16 h-16 text-zinc-600 mx-auto mb-4" />
            <p className="text-zinc-400">Aún no tienes pedidos</p>
          </div>
        ) : (
                ...
                <motion.div key={order?.id ?? i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
                    <div>
                      ...
                      <p className="text-xs text-zinc-500">{order?.createdAt ? new Date(order.createdAt).toLocaleDateString("es-ES") : ""}</p>
                    </div>
