...
export function ProductDetailClient({ productId }: Readonly<{ productId: string }>) {
  ...
  const matConfig = useMemo(() => {
    ...
    if (fromDB) return { pricePerKg: fromDB.pricePerKg, density: fromDB.density, maintenanceFactor: fromDB.maintenanceFactor };
    const fromStatic = MATERIAL_INFO[selectedMaterial as keyof typeof MATERIAL_INFO] ?? MATERIAL_INFO.PLA;
  ...
  const handleAddToCart = () => {
    ...
    if (isAdmin) { showToast("warning", t.adminNoPurchase); return; }
    addItem?.({
              ...
              {priceCalc && (
                <div className="grid grid-cols-3 gap-2 text-center">
                  {[
                    ...
                    <div key={label} className={`rounded-lg p-2 border transition-all ${active ? "border-cyan bg-cyan/5" : "border-white/5 opacity-50"}`}>
