...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const review = await prisma.review.create({
      data: { userId, productId: params?.id ?? "", rating: Math.min(5, Math.max(1, rating)), comment },
      include: { user: { select: { name: true, email: true } } },
    });
    const agg = await prisma.review.aggregate({ where: { productId: params?.id }, _avg: { rating: true }, _count: true });
