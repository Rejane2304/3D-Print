...
const MaterialColorsManager = dynamic(
  () => import("./material-colors-manager"),
  { ssr: false },
);
...
export default function AdminMaterialsClient() {
  ...
  const handleDelete = async (id: string) => {
    if (
      !confirm("¿Eliminar este material? Se eliminarán sus precios asociados.")
    )
      return;
  ...
  const handleRecalculate = async () => {
    ...
    try {
      ...
      notify(
        "success",
        `Precios recalculados: ${data.updated ?? 0} productos actualizados`,
      );
    } catch {
    ...
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <Layers className="w-6 h-6 text-cyan" /> Materiales
          </h1>
          <p className="text-sm text-zinc-400 mt-1">
            Gestiona los materiales y sus parámetros de coste. Los cambios
            actualizan los precios automáticamente.
          </p>
        </div>
        <div className="flex gap-2">
          <button
            ...
            className="flex items-center gap-2 px-3 py-2 bg-white/5 rounded-lg text-sm hover:bg-white/10 transition disabled:opacity-50"
          >
            <RefreshCw
      ...
      {showForm && (
        <div className="bg-bg-card border border-white/10 rounded-xl p-6 mb-6">
          ...
          {/* Gestión de colores por material solo al editar */}
          {editing && <MaterialColorsManager materialId={editing} />}
        </div>
      )}
            ...
            <tbody className="divide-y divide-white/5">
              {materials.map((m) => (
                <tr key={m.id} className="hover:bg-white/[0.02] transition">
                  ...
                  <td className="px-4 py-3 text-center">
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full ${m.inStock ? "bg-green-500/10 text-green-400" : "bg-red-500/10 text-red-400"}`}
                    >
              ...
              {materials.length === 0 && (
                <tr>
                  ...
                  >
                    No hay materiales. Crea el primero.
                  </td>
                </tr>
              )}
            </tbody>
