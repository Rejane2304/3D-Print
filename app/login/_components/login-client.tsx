  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        else { showToast("error", t.invalidCredentials); }
      } else {
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input type="text" value={name} onChange={e => { setName(e?.target?.value ?? ""); setErrors(prev => { const n = { ...prev }; delete n.name; return n; }); }}
                    placeholder={t.namePlaceholder} className={`w-full bg-white/5 rounded-lg pl-10 pr-4 py-2.5 text-sm outline-none focus:ring-1 placeholder-zinc-500 ${getFieldState("name") === "error" ? "ring-1 ring-red-400" : "focus:ring-cyan"}`} />
                </div>
