...
describe('Products API Integration', () => {
    describe('GET /api/products', () => {
        ...
        it('should filter by material', async () => {
            const { GET } = await import('@/app/api/products/route');
            const request = { url: 'http://localhost:3000/api/products?material=PLA' };
            const response = await GET(request);
            const data = await response.json();
            expect(response.status).toBe(200);
            expect(data.products).toBeDefined();
        ...
        it('should filter by category', async () => {
            const { GET } = await import('@/app/api/products/route');
            const request = { url: 'http://localhost:3000/api/products?category=Decoracion' };
            const response = await GET(request);
            expect(response.status).toBe(200);
        });
        it('should support pagination', async () => {
            const { GET } = await import('@/app/api/products/route');
            const request = { url: 'http://localhost:3000/api/products?page=1&limit=10' };
            const response = await GET(request);
            const data = await response.json();
            expect(response.status).toBe(200);
    ...
    describe('GET /api/products/[id]', () => {
        it('should return a single product', async () => {
            ...
            const request = { url: 'http://localhost:3000/api/products/1' };
        ...
        it('should return 404 for non-existent product', async () => {
            const { GET } = await import('@/app/api/products/[id]/route');
            const request = { url: 'http://localhost:3000/api/products/999' };
