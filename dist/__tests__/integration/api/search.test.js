...
const mockProducts = [
    ...
    { id: '3', name: 'Soporte Teléfono', category: 'Accesorios', material: 'PLA', basePricePerGram: 0.05, images: [] },
];
...
describe('Search API Integration', () => {
    describe('GET /api/search', () => {
        it('should return products matching search query', async () => {
            ...
            const request = { url: 'http://localhost:3000/api/search?q=soporte' };
        ...
        it('should return empty array for no matches', async () => {
            const { GET } = await import('@/app/api/search/route');
            const request = { url: 'http://localhost:3000/api/search?q=zzzznotfound' };
            const response = await GET(request);
            const data = await response.json();
            expect(response.status).toBe(200);
            expect(Array.isArray(data)).toBe(true);
        ...
        it('should limit results', async () => {
            const { GET } = await import('@/app/api/search/route');
            const request = { url: 'http://localhost:3000/api/search?q=soporte&limit=1' };
            const response = await GET(request);
            const data = await response.json();
            expect(response.status).toBe(200);
        ...
        it('should return error for empty query', async () => {
            const { GET } = await import('@/app/api/search/route');
            const request = { url: 'http://localhost:3000/api/search?q=' };
            const response = await GET(request);
            expect(response.status).toBe(200);
        });
    });
