...
const Carousel = React.forwardRef(({ orientation = 'horizontal', opts, setApi, plugins, className, children, ...props }, ref) => {
    ...
    return (<CarouselContext.Provider value={contextValue}>
        <section ref={ref} onKeyDownCapture={handleKeyDown} className={cn('relative', className)} aria-roledescription="carousel" {...props}>
          {children}
        </section>
...
const CarouselItem = React.forwardRef(({ className, ...props }, ref) => {
    ...
    return (<article ref={ref} aria-roledescription="slide" className={cn('min-w-0 shrink-0 grow-0 basis-full', orientation === 'horizontal' ? 'pl-4' : 'pt-4', className)} {...props}/>);
});
...
const CarouselPrevious = React.forwardRef(({ className, variant = 'outline', size = 'icon', ...props }, ref) => {
    ...
    return (<Button ref={ref} variant={variant} size={size} className={cn('absolute  h-8 w-8 rounded-full', orientation === 'horizontal'
      ...
      <span className="sr-only">{language === 'es' ? 'Diapositiva anterior' : 'Previous slide'}</span>
    </Button>);
});
...
const CarouselNext = React.forwardRef(({ className, variant = 'outline', size = 'icon', ...props }, ref) => {
    ...
    return (<Button ref={ref} variant={variant} size={size} className={cn('absolute h-8 w-8 rounded-full', orientation === 'horizontal'
      ...
      <span className="sr-only">{language === 'es' ? 'Diapositiva siguiente' : 'Next slide'}</span>
    </Button>);
});
