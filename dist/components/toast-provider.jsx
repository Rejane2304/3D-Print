      ...
      <div className="fixed top-20 right-4 z-[100] flex flex-col gap-2 max-w-sm w-full pointer-events-none">
        <AnimatePresence>
          {(toasts ?? []).map(toast => {
            ...
            return (<motion.div key={toast?.id} initial={{ opacity: 0, x: 100 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 100 }} className={`pointer-events-auto flex items-center gap-3 p-4 rounded-lg border ${colors[toast?.type ?? "info"]} backdrop-blur-md`}>
