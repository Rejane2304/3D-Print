 ...
 */
export function calculateWeight(dimX, dimY, dimZ, density, fillFactor = PRICING_CONFIG.infillFactor) {
 ...
 */
export function scalePrintTime(basePrintTimeMinutes, dimX, dimY, dimZ, refDimX, refDimY, refDimZ) {
    const refVol = refDimX * refDimY * refDimZ;
    if (refVol <= 0)
        return basePrintTimeMinutes;
 ...
 */
export function calculateAdvancedPrice(weightGrams, printTimeMinutes, material, quantity = 1, config) {
    const printTimeHours = printTimeMinutes / 60;
    const weightKg = weightGrams / 1000;
    const machineAmortizationPerHour = config?.machineAmortizationPerHour ?? PRICING_CONFIG.machineAmortizationPerHour;
    const operationCostPerHour = config?.operationCostPerHour ?? PRICING_CONFIG.operationCostPerHour;
    const consumablesCostPerHour = config?.consumablesCostPerHour ?? PRICING_CONFIG.consumablesCostPerHour;
 ...
 */
export function calculatePriceFromDimensions(dimX, dimY, dimZ, basePrintTimeMinutes, material, options) {
    ...
    const printTimeMinutes = options?.refDimX !== undefined &&
        options.refDimY !== undefined &&
        options.refDimZ !== undefined
        ? scalePrintTime(basePrintTimeMinutes, dimX, dimY, dimZ, options.refDimX, options.refDimY, options.refDimZ)
 ...
 */
export function getPriceByQuantity(priceUnit, priceMedium, priceBulk, quantity) {
    if (quantity >= 10)
        return priceBulk;
    if (quantity >= 5)
        return priceMedium;
