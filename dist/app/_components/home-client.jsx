...
export function HomeClient() {
    ...
    const tHero = {
        es: {
            ...
            subtitle: "Personaliza cada pieza eligiendo material, color y dimensiones. Tecnología de impresión 3D de alta calidad con materiales PLA y PETG.",
        ...
        en: {
            ...
            subtitle: "Customize each part by choosing material, color and dimensions. High‑quality 3D printing with PLA and PETG materials.",
            cta: "Browse catalog",
        },
    }[language];
    const tStats = {
    ...
    const tMaterials = {
        es: {
            ...
            plaDesc: "Material rígido, con buen detalle y acabado visual. Perfecto para macetas, figuras y accesorios ligeros.",
            petgDesc: "Material más resistente y flexible, ideal para soportes, piezas sometidas a uso frecuente y exteriores.",
            seeProducts: (mat) => `Ver productos ${mat}`,
            idealFor: "Ideal para",
        },
        en: {
            ...
            plaDesc: "Rigid material with good detail and visual finish. Perfect for pots, figures and light accessories.",
            petgDesc: "Stronger and more flexible material, ideal for mounts, parts under frequent use and outdoor pieces.",
            seeProducts: (mat) => `View ${mat} products`,
            idealFor: "Ideal for",
        },
    }[language];
    const tBenefits = {
      ...
      <section className="py-20">
        <div className="max-w-site mx-auto px-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {(featured ?? []).map((p, i) => (<motion.div key={p?.id ?? i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }}>
                  ...
                  <div className="bg-bg-card rounded-lg overflow-hidden card-shadow card-shadow-hover transition-all duration-300">
                    ...
                    <div className="p-4">
                      ...
                      {(p?.colors?.length ?? 0) > 0 && (<div className="flex items-center gap-1 mb-2">
                          {(p?.colors ?? []).slice(0, 3).map((color) => (<span key={color} className="w-4 h-4 rounded-full border border-white/20" style={{ backgroundColor: MATERIAL_INFO[p?.material ?? "PLA"]?.color ?? "#6B7280" }} title={color}/>))}
                ...
                const price = require("@/lib/price-calculator").calculatePriceFromDimensions(p?.defaultDimX ?? 0, p?.defaultDimY ?? 0, p?.defaultDimZ ?? 0, p?.printTimeMinutes ?? 60, mat, {
      ...
      <section className="py-20 bg-[#0d0d0d]">
        <div className="max-w-site mx-auto px-4">
          ...
          <div className="grid md:grid-cols-2 gap-8">
            ...
            return (<motion.div key={mat} initial={{ opacity: 0, x: i === 0 ? -30 : 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="bg-bg-card rounded-xl p-6 card-shadow hover:shadow-lg transition-all duration-300 border border-white/5">
        ...
        <div className="max-w-site mx-auto px-4">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-12">
            <h2 className="text-3xl font-bold mb-3">
              {language === "es" ? (<>¿Por qué <span className="text-gradient-cyan">3D Print</span>?</>) : (<>Why <span className="text-gradient-cyan">3D Print</span>?</>)}
            </h2>
          </motion.div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {tBenefits.map((b) => (<motion.div key={b.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: tBenefits.indexOf(b) * 0.1 }} className="bg-bg-card rounded-xl p-6 card-shadow hover:shadow-lg transition-all duration-300 border border-white/5 text-center">
