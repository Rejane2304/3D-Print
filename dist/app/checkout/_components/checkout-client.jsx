...
export function CheckoutClient() {
    ...
    const validateShipping = () => {
        ...
        if (!shipping.address?.trim())
            errs.address = t.addressRequired;
        if (!shipping.city?.trim())
            errs.city = t.cityRequired;
        if (!shipping.zip?.trim())
            errs.zip = t.zipRequired;
        setErrors(errs);
            ...
            const res = await fetch("/api/checkout", {
                ...
                body: JSON.stringify({
                    items: (items ?? []).map(i => ({
                        ...
                        color: i?.color, quantity: i?.quantity, dimX: i?.dimX,
    ...
    if ((items?.length ?? 0) === 0)
      ...
      <div className="text-center">
        ...
        <a href="/catalog" className="text-cyan hover:underline">{t.goCatalog}</a>
      </div>
    </div>);
    const fieldClass = (field) => `w-full bg-white/5 rounded-lg px-4 py-2.5 text-sm outline-none focus:ring-1 placeholder-zinc-500 ${errors?.[field] ? "ring-1 ring-red-400" : "focus:ring-cyan"}`;
    return (<div className="min-h-screen py-8">
      <div className="max-w-3xl mx-auto px-4">
        ...
        <div className="flex items-center gap-4 mb-10">
          {STEPS[language].map((s, i) => (<React.Fragment key={s.label}>
        ...
        {/* Step 1: Shipping */}
        {step === 0 && (<motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="bg-bg-card rounded-xl p-6 border border-white/5">
        ...
        {/* Step 2: Summary */}
        {step === 1 && (<motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="bg-bg-card rounded-xl p-6 border border-white/5">
            <h2 className="text-xl font-semibold mb-6 flex items-center gap-2">
              <Package className="w-5 h-5 text-cyan"/> {t.summaryTitle}
            </h2>
            <div className="space-y-3 mb-6">
              {(items ?? []).map((item, i) => (<div key={item?.id ?? i} className="flex items-center gap-3 p-3 bg-white/5 rounded-lg">
            ...
            <div className="mb-4">
              ...
              {couponCode ? (<div className="flex items-center gap-2 p-2.5 bg-green-500/10 border border-green-500/30 rounded-lg">
                  ...
                  <input type="text" value={couponInput} onChange={e => setCouponInput(e.target.value.toUpperCase())} onKeyDown={e => e.key === "Enter" && handleApplyCoupon()} placeholder={t.couponPlaceholder} className="flex-1 bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm font-mono uppercase outline-none focus:ring-1 focus:ring-cyan placeholder-zinc-600"/>
            ...
            <div className="space-y-2 text-sm">
              ...
              {couponDiscount > 0 && (<div className="flex justify-between text-green-400">
            ...
            <div className="mt-6 p-4 bg-white/5 rounded-lg text-sm">
              ...
              <p className="text-zinc-400">{shipping.name} · {shipping.address}, {shipping.zip} {shipping.city}, {shipping.country}</p>
            </div>
          </motion.div>)}

        {/* Step 3: Payment */}
        {step === 2 && (<motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="bg-bg-card rounded-xl p-6 border border-white/5">
            <h2 className="text-xl font-semibold mb-6 flex items-center gap-2">
            ...
            <div className="p-6 bg-white/5 rounded-lg text-center">
              ...
              {couponDiscount > 0 && (<p className="text-sm text-green-400 mb-2 flex items-center justify-center gap-1">
        ...
        <div className="flex justify-between mt-8">
          <button onClick={() => setStep(Math.max(0, step - 1))} disabled={step === 0} className="flex items-center gap-2 px-4 py-2 bg-bg-card border border-white/5 rounded-lg text-sm hover:bg-bg-hover transition disabled:opacity-30">
            <ArrowLeft className="w-4 h-4"/> {t.prev}
          </button>
          {step < 2 && (<button onClick={handleNext} className="flex items-center gap-2 px-6 py-2 bg-cyan text-black rounded-lg text-sm font-medium hover:bg-cyan-dim transition">
