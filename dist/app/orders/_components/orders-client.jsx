...
export function OrdersClient() {
    ...
    return (<div className="min-h-screen py-8">
      <div className="max-w-site mx-auto px-4">
        ...
        {(orders?.length ?? 0) === 0 ? (<div className="text-center py-20">
            ...
            {(orders ?? []).map((order, i) => {
                ...
                return (<motion.div key={order?.id ?? i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="bg-bg-card rounded-xl p-5 border border-white/5">
                  ...
                  {!isCancelled && order?.status !== "pending" && (<div className="flex items-center gap-0 mb-4 overflow-x-auto pb-1">
                      {WORKFLOW_STEPS.map((step, idx) => {
                            ...
                            {idx < WORKFLOW_STEPS.length - 1 && (<div className={`flex-1 h-px min-w-4 mx-1 mb-4 ${isDone ? "bg-white/20" : "bg-white/5"}`}/>)}
                          </React.Fragment>);
                        })}
                    </div>)}

                  <div className="space-y-2">
                    {(order?.items ?? []).map((item, j) => (<div key={item?.id ?? j} className="flex justify-between text-sm">
