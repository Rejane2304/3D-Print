...
const WORKFLOW_STEPS = ['paid', 'processing', 'ready', 'shipped', 'delivered'];
function stepBadgeClass(isActive, isDone, activeColor) {
    if (isActive)
        return activeColor;
    if (isDone)
...
export default function AdminOrdersClient() {
    ...
    if (loading) {
        orderListContent = (<div className="flex items-center justify-center py-12">
        <div className="w-8 h-8 border-2 border-cyan border-t-transparent rounded-full animate-spin"/>
      </div>);
    }
    else if (orders.length === 0) {
        orderListContent = (<div className="text-center py-12">
        <ShoppingCart className="w-12 h-12 text-muted mx-auto mb-4"/>
        <p className="text-muted">No se encontraron pedidos</p>
      </div>);
    }
    else {
        orderListContent = (<div className="overflow-x-auto">
        <table className="w-full">
          ...
          <tbody>
            {orders.map((order) => {
                ...
                return (<tr key={order.id} className="border-t border-border hover:bg-bg-tertiary/50">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-end gap-2">
                      <button onClick={() => setSelectedOrder(order)} className="p-2 hover:bg-cyan/20 rounded-lg transition-colors">
    ...
    return (<div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <input type="text" placeholder="Buscar por ID, email o nombre..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-10 pr-4 py-3 bg-bg-secondary border border-border rounded-lg focus:outline-none focus:border-cyan"/>
        </div>
        <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="px-4 py-3 bg-bg-secondary border border-border rounded-lg focus:outline-none focus:border-cyan">
      ...
      {/* Order Detail Modal */}
      <AnimatePresence>
        {selectedOrder && (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" onClick={() => setSelectedOrder(null)}>
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="bg-bg-secondary border border-border rounded-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
              <div className="flex items-center justify-between p-6 border-b border-border">
                <div>
                  <h2 className="text-xl font-bold">Pedido #{selectedOrder.id.slice(-8).toUpperCase()}</h2>
                  <p className="text-sm text-muted">
                    {new Date(selectedOrder.createdAt).toLocaleString('es-ES')}
                  </p>
                </div>
                <button onClick={() => setSelectedOrder(null)} className="p-2 hover:bg-bg-tertiary rounded-lg">
                  <X className="w-5 h-5"/>
                </button>
              </div>
              ...
              <div className="p-6 space-y-6">
                ...
                const isDone = arr.indexOf(selectedOrder.status) > i;
                ...
                return (<div key={s} className="flex items-center gap-1">
                  ...
                  {workflowNext[selectedOrder.status] && (<button onClick={advanceWorkflow} disabled={advancing} className="w-full py-2.5 bg-cyan text-black font-semibold rounded-lg hover:bg-cyan-dim transition text-sm disabled:opacity-50">
                ...
                <div>
                  ...
                  <select id="status-select" value={selectedOrder.status} onChange={(e) => updateStatus(selectedOrder.id, e.target.value)} className="w-full px-4 py-2 bg-bg-tertiary border border-border rounded-lg focus:outline-none focus:border-cyan">
                    {Object.entries(statusConfig).map(([key, { label }]) => (<option key={key} value={key}>{label}</option>))}
                  </select>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    {selectedOrder.items.map((item) => (<div key={item.id} className="flex justify-between items-center py-2 border-b border-border last:border-0">
                        <div>
