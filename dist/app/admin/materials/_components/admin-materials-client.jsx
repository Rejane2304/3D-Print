...
export default function AdminMaterialsClient() {
    ...
    const handleDelete = async (id) => {
        if (!confirm('¿Eliminar este material? Se eliminarán sus precios asociados.'))
            return;
    ...
    return (<div className="p-6 max-w-5xl mx-auto">
      ...
      {showForm && (<div className="bg-bg-card border border-white/10 rounded-xl p-6 mb-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            ...
            ].map(({ key, label, placeholder, type, disabled }) => (<div key={key}>
                <label className="text-xs text-zinc-400 block mb-1">{label}</label>
                <input type={type} placeholder={placeholder} disabled={disabled} value={form[key]} onChange={e => setForm(f => ({ ...f, [key]: e.target.value }))} className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm outline-none focus:ring-1 focus:ring-cyan disabled:opacity-40"/>
              </div>))}
            <div>
      ...
      {loading ? (<div className="flex justify-center py-12"><div className="w-6 h-6 border-2 border-cyan border-t-transparent rounded-full animate-spin"/></div>) : (<div className="bg-bg-card border border-white/5 rounded-xl overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-white/5">
              {materials.map(m => (<tr key={m.id} className="hover:bg-white/[0.02] transition">
              ...
              {materials.length === 0 && (<tr><td colSpan={6} className="px-4 py-8 text-center text-zinc-500 text-sm">No hay materiales. Crea el primero.</td></tr>)}
            </tbody>
          </table>
        </div>)}
    </div>);
