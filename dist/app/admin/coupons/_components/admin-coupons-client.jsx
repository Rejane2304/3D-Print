...
export default function AdminCouponsClient() {
    ...
    const isExhausted = (c) => c.maxUses !== null && c.usedCount >= (c.maxUses ?? 0);
    return (<div className="p-6 max-w-5xl mx-auto">
      ...
      {feedback && (<div className={`flex items-center gap-2 p-3 rounded-lg mb-4 text-sm ${feedback.type === 'success' ? 'bg-green-500/10 text-green-400' : 'bg-red-500/10 text-red-400'}`}>
          {feedback.type === 'success' ? <CheckCircle className="w-4 h-4"/> : <AlertTriangle className="w-4 h-4"/>}
          {feedback.msg}
        </div>)}

      {showForm && (<div className="bg-bg-card border border-white/10 rounded-xl p-6 mb-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            <div>
              ...
              <input id="couponCode" type="text" placeholder="VERANO20" disabled={!!editing} value={form.code} onChange={e => setForm(f => ({ ...f, code: e.target.value.toUpperCase() }))} className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm font-mono outline-none focus:ring-1 focus:ring-cyan disabled:opacity-40 uppercase"/>
            </div>
            <div>
            ...
            <div>
              ...
              <input id="couponDiscountValue" type="number" placeholder={form.discountType === 'percentage' ? '10' : '5'} min="0" value={form.discountValue} onChange={e => setForm(f => ({ ...f, discountValue: e.target.value }))} className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm outline-none focus:ring-1 focus:ring-cyan"/>
            </div>
            <div>
              <label htmlFor="couponMinPurchase" className="text-xs text-zinc-400 block mb-1">Compra mínima (€)</label>
              <input id="couponMinPurchase" type="number" placeholder="0 (sin límite)" min="0" value={form.minPurchase} onChange={e => setForm(f => ({ ...f, minPurchase: e.target.value }))} className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm outline-none focus:ring-1 focus:ring-cyan"/>
            </div>
            <div>
              <label htmlFor="couponMaxUses" className="text-xs text-zinc-400 block mb-1">Usos máximos</label>
              <input id="couponMaxUses" type="number" placeholder="Ilimitado" min="1" value={form.maxUses} onChange={e => setForm(f => ({ ...f, maxUses: e.target.value }))} className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm outline-none focus:ring-1 focus:ring-cyan"/>
            </div>
            <div>
              <label htmlFor="couponValidFrom" className="text-xs text-zinc-400 block mb-1">Válido desde</label>
              <input id="couponValidFrom" type="date" value={form.validFrom} onChange={e => setForm(f => ({ ...f, validFrom: e.target.value }))} className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm outline-none focus:ring-1 focus:ring-cyan"/>
            </div>
            <div>
              <label htmlFor="couponValidUntil" className="text-xs text-zinc-400 block mb-1">Válido hasta (opcional)</label>
              <input id="couponValidUntil" type="date" value={form.validUntil} onChange={e => setForm(f => ({ ...f, validUntil: e.target.value }))} className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-sm outline-none focus:ring-1 focus:ring-cyan"/>
            </div>
      ...
      {loading ? (<div className="flex justify-center py-12"><div className="w-6 h-6 border-2 border-cyan border-t-transparent rounded-full animate-spin"/></div>) : (<div className="bg-bg-card border border-white/5 rounded-xl overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-white/5">
              {coupons.map(c => (<tr key={c.id} className="hover:bg-white/[0.02] transition">
                  ...
                  <td className="px-4 py-3 text-center">
                    {!c.isActive && (<span className="text-xs px-2 py-0.5 rounded-full bg-zinc-700/20 text-zinc-400">Inactivo</span>)}
                    {c.isActive && isExpired(c) && (<span className="text-xs px-2 py-0.5 rounded-full bg-red-500/10 text-red-400">Expirado</span>)}
                    {c.isActive && isExhausted(c) && !isExpired(c) && (<span className="text-xs px-2 py-0.5 rounded-full bg-amber/10 text-amber">Agotado</span>)}
                    {c.isActive && !isExpired(c) && !isExhausted(c) && (<span className="text-xs px-2 py-0.5 rounded-full bg-green-500/10 text-green-400">Activo</span>)}
                  </td>
              ...
              {coupons.length === 0 && (<tr><td colSpan={6} className="px-4 py-8 text-center text-zinc-500 text-sm">No hay cupones. Crea el primero.</td></tr>)}
            </tbody>
          </table>
        </div>)}
    </div>);
