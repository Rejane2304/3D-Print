...
export function ProductDetailClient({ productId }) {
    ...
    const t = {
        es: {
            ...
            addedToCart: (name) => `${name} añadido al carrito`,
            productNotFound: "Producto no encontrado",
            backToCatalog: "Volver al catálogo",
            breadcrumbHome: "Inicio",
            breadcrumbCatalog: "Catálogo",
            ratingLabel: (count) => `${count} reseñas`,
            ...
            priceHint: "Precio calculado según material, dimensiones y cantidad. Descuentos por volumen disponibles.",
            addToCart: "Añadir al Carrito",
            faqTitle: "Preguntas Frecuentes",
            reviewsTitle: "Reseñas", writeReviewTitle: "Escribe una reseña",
            reviewPlaceholder: "Comparte tu experiencia...",
            publishing: "Publicando...", publishReview: "Publicar reseña",
        ...
        en: {
            ...
            ratingLabel: (count) => `${count} reviews`,
            materialLabel: "Material",
            colorLabel: "Color",
            ...
            priceHint: "Price calculated by material, dimensions and quantity. Volume discounts available.",
            addToCart: "Add to cart",
            faqTitle: "Frequently Asked Questions",
            reviewsTitle: "Reviews", writeReviewTitle: "Write a review",
            reviewPlaceholder: "Share your experience...",
            publishing: "Publishing...", publishReview: "Publish review",
    ...
    const matConfig = useMemo(() => {
        ...
        if (fromDB)
            return { pricePerKg: fromDB.pricePerKg, density: fromDB.density, maintenanceFactor: fromDB.maintenanceFactor };
        const fromStatic = MATERIAL_INFO[selectedMaterial] ?? MATERIAL_INFO.PLA;
        return { pricePerKg: fromStatic.pricePerKg, density: fromStatic.density, maintenanceFactor: fromStatic.maintenanceFactor };
    }, [materials, selectedMaterial]);
    const priceCalc = useMemo(() => {
        if (!product)
            return null;
        return calculatePriceFromDimensions(dimX, dimY, dimZ, product.printTimeMinutes ?? 60, matConfig, {
            quantity,
    ...
    const handleSubmitReview = async () => {
        if (!session?.user || !reviewText?.trim())
            return;
    ...
    const clampDim = (val, min, max) => Math.max(min, Math.min(max, val));
    ...
    const faqs = language === "es"
        ? [
            ...
            { q: "¿Cómo se calcula el precio?", a: "El precio incluye material, amortización de máquina (P2S), mantenimiento y electricidad. Hay descuentos por volumen para pedidos de 5 o más unidades." },
        ]
        : [
            { q: "How long does production take?", a: "Standard lead time is 2–5 business days depending on part size and complexity." },
            { q: "Can I request custom dimensions?", a: "Yes, configure exact dimensions with the configurator. Dimensions are limited by the printer capabilities." },
            ...
            { q: "How is the price calculated?", a: "Price includes material, machine amortization (P2S), maintenance and electricity. Volume discounts apply for 5+ units." },
        ];
      ...
      <div className="max-w-site mx-auto px-4">
        ...
        <div className="grid lg:grid-cols-2 gap-10">
          ...
          <div>
            ...
            {product.images?.length > 1 && (<div className="flex gap-2 overflow-x-auto scrollbar-hide">
                {product.images.map((img, i) => (<button key={img} onClick={() => setSelectedImage(i)} className={`relative w-20 h-14 rounded-lg overflow-hidden flex-shrink-0 border-2 transition ${selectedImage === i ? "border-cyan" : "border-transparent opacity-60 hover:opacity-100"}`}>
          ...
          <div>
            <div className="flex items-center gap-2 mb-2">
              <span className={`text-xs font-medium px-2 py-0.5 rounded-full border ${getBadgeClass(product.material)}`}>{product.material}</span>
            ...
            {product.reviewCount > 0 && (<div className="flex items-center gap-2 mb-6">
            ...
            return (<button key={code} onClick={() => setSelectedMaterial(code)} className={`p-3 rounded-lg border-2 transition-all text-left ${btnBorderClass}`} style={isSelected ? { borderColor: matColor, backgroundColor: `${matColor}18` } : {}}>
                        <div className="flex items-center gap-2">
                          <Layers className="w-4 h-4" style={{ color: matColor }}/>
                          <span className="font-semibold text-sm">{code}</span>
                        </div>
                        {info?.uses && (<p className="text-xs text-zinc-400 mt-1 line-clamp-2">{info.uses}</p>)}
                      </button>);
        })}
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {product.colors.slice(0, 8).map(color => (<button key={color} onClick={() => setSelectedColor(color)} className={`w-9 h-9 rounded-full border-2 transition-all ${selectedColor === color ? "scale-110 ring-2 ring-cyan/30 border-cyan" : "border-white/20 hover:border-white/40"}`} style={{ backgroundColor: COLOR_MAP[color] ?? "#808080" }} title={color} aria-label={`${t.colorLabel} ${color}`}/>))}
                </div>
              </div>
            ...
            { label: t.dimZLabel, val: dimZStr, setVal: setDimZStr, setMm: setDimZ, minMm: product.minDimZ ?? 10 },
                      ...
                      <input type="number" min={mmToCm(d.minMm)} max={MAX_DIM_CM} step={0.5} value={Number(d.val).toFixed(2)} onChange={e => d.setVal(e.target.value)} onBlur={e => {
                const raw = Number.parseFloat(e.target.value);
                const clampedMm = clampDim(cmToMm(Number.isNaN(raw) ? mmToCm(d.minMm) : raw), d.minMm, cmToMm(MAX_DIM_CM));
                d.setMm(clampedMm);
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  {quantity >= 10 && <span className="text-xs text-green-400 flex items-center gap-1"><TrendingDown className="w-3 h-3"/>–40%</span>}
                  {quantity >= 5 && quantity < 10 && <span className="text-xs text-green-400 flex items-center gap-1"><TrendingDown className="w-3 h-3"/>–20%</span>}
                </div>
              </div>

              {/* Tiered Pricing */}
              {priceCalc && (<div className="grid grid-cols-3 gap-2 text-center">
                  {[
                { label: t.tieredUnit, price: priceCalc.priceUnit, active: quantity < 5 },
                { label: t.tieredMedium, price: priceCalc.priceMedium, active: quantity >= 5 && quantity < 10 },
                { label: t.tieredBulk, price: priceCalc.priceBulk, active: quantity >= 10 },
            ].map(({ label, price, active }) => (<div key={label} className={`rounded-lg p-2 border transition-all ${active ? "border-cyan bg-cyan/5" : "border-white/5 opacity-50"}`}>
                      <p className="text-[10px] text-zinc-400 mb-0.5">{label}</p>
                      <p className={`font-mono text-sm font-semibold ${active ? "text-cyan" : "text-zinc-300"}`}>€{price.toFixed(2)}</p>
                    </div>))}
                </div>)}

              {/* Price Summary */}
              {priceCalc && (<div className="bg-white/5 rounded-lg p-4 space-y-2">
                  ...
                  {quantity > 1 && (<p className="text-xs text-zinc-500 font-mono">€{priceCalc.finalPrice.toFixed(2)} × {quantity}</p>)}
                  ...
                  {isAdmin && (<>
                      ...
                      <AnimatePresence>
                        {showBreakdown && (<motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                    ...
                    ].map(({ label, value }) => (<div key={label} className="flex justify-between text-xs">
              ...
              {/* Add to Cart */}
              <button onClick={handleAddToCart} className="w-full py-3 bg-cyan text-black font-semibold rounded-lg hover:bg-cyan-dim transition flex items-center justify-center gap-2 disabled:opacity-50" disabled={isAdmin} title={isAdmin ? t.adminNoPurchase : undefined}>
        ...
        <section className="mt-16">
          ...
          <div className="space-y-3">
            {faqs.map((faq, i) => (<div key={faq.q} className="bg-bg-card rounded-xl border border-white/5 overflow-hidden">
                ...
                <AnimatePresence>
                  {faqOpen === i && (<motion.div initial={{ height: 0 }} animate={{ height: "auto" }} exit={{ height: 0 }} className="overflow-hidden">
        ...
        <section className="mt-16 mb-16">
          <h2 className="text-2xl font-bold mb-6">{t.reviewsTitle} ({product.reviewCount})</h2>
          {session?.user && (<div className="bg-bg-card rounded-xl p-6 border border-white/5 mb-6">
              <h3 className="font-semibold mb-3 text-sm">{t.writeReviewTitle}</h3>
              <div className="flex gap-1 mb-3">
                {Array.from({ length: 5 }).map((_, i) => (<button key={"wstar-" + i} onClick={() => setReviewRating(i + 1)} aria-label={t.starsAria(i + 1)}>
                    <Star className={`w-5 h-5 ${i < reviewRating ? "fill-amber text-amber" : "text-zinc-600"}`}/>
                  </button>))}
              </div>
              <textarea value={reviewText} onChange={e => setReviewText(e.target.value)} placeholder={t.reviewPlaceholder} className="w-full bg-white/5 rounded-lg p-3 text-sm outline-none focus:ring-1 focus:ring-cyan placeholder-zinc-500 resize-none h-20"/>
          ...
          <div className="space-y-4">
            {product.reviews.map((r, i) => (<motion.div key={r?.id ?? i} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="bg-bg-card rounded-xl p-5 border border-white/5">
