...
export function CartClient() {
    ...
    const t = {
        es: {
            ...
            back: "Volver",
            items: (n) => `${n} producto${n === 1 ? "" : "s"} en tu carrito`,
        ...
        en: {
            ...
            title: "Shopping Cart",
            back: "Back",
            items: (n) => `${n} item${n === 1 ? "" : "s"} in your cart`,
    ...
    return (<div className="min-h-screen py-8">
      <div className="max-w-site mx-auto px-4">
        <div className="flex items-center justify-between gap-4 mb-2">
          ...
          <button type="button" onClick={() => router.push('/catalog')} className="inline-flex items-center gap-2 px-3 py-2 text-sm rounded-lg border border-white/10 hover:bg-white/5 transition">
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-4">
            {(items ?? []).map((item, i) => (<motion.div key={item?.id ?? i} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }} className="bg-bg-card rounded-xl p-4 border border-white/5 flex gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <button onClick={() => updateQuantity?.(item?.id ?? "", (item?.quantity ?? 1) - 1)} className="w-7 h-7 rounded bg-white/5 flex items-center justify-center hover:bg-white/10 transition" aria-label={t.reduceQty}>
                        <Minus className="w-3 h-3"/>
                      </button>
                      <span className="font-mono text-sm w-6 text-center">{item?.quantity ?? 1}</span>
                      <button onClick={() => updateQuantity?.(item?.id ?? "", (item?.quantity ?? 1) + 1)} className="w-7 h-7 rounded bg-white/5 flex items-center justify-center hover:bg-white/10 transition" aria-label={t.increaseQty}>
                        <Plus className="w-3 h-3"/>
                      </button>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className="font-mono text-sm text-cyan">€{((item?.unitPrice ?? 0) * (item?.quantity ?? 1)).toFixed(2)}</span>
                      <button onClick={() => { removeItem?.(item?.id ?? ""); showToast("info", t.removed); }} className="p-1.5 rounded hover:bg-red-400/10 text-zinc-400 hover:text-red-400 transition" aria-label={t.removeItem}>
          ...
          <div className="lg:col-span-1">
            <div className="bg-bg-card rounded-xl p-6 border border-white/5 card-shadow sticky top-24">
              ...
              <div className="space-y-3 text-sm">
                <div className="flex justify-between"><span className="text-zinc-400">{t.subtotal}</span><span className="font-mono">€{subtotal.toFixed(2)}</span></div>
                <div className="flex justify-between"><span className="text-zinc-400">{t.tax}</span><span className="font-mono">€{tax.toFixed(2)}</span></div>
                <div className="flex justify-between"><span className="text-zinc-400">{t.shipping}</span><span className="font-mono">{shipping === 0 ? t.free : `€${shipping.toFixed(2)}`}</span></div>
              ...
              {session?.user ? (<Link href="/checkout" className="block w-full mt-6 py-3 bg-cyan text-black font-semibold rounded-lg hover:bg-cyan-dim transition text-sm text-center">
                  {t.checkout}
                </Link>) : (<Link href="/login" className="block w-full mt-6 py-3 bg-cyan text-black font-semibold rounded-lg hover:bg-cyan-dim transition text-sm text-center">
                  {t.loginToBuy}
                </Link>)}
            </div>
          </div>
