...
const CATEGORIES = ["Todos", "Accesorios", "Decoracion", "Figuras", "Funcional", "Articulados"];
...
export function CatalogClient() {
    ...
    const t = {
        es: {
            ...
            activeFiltersMaterial: (m) => m,
            activeFiltersCategory: (c) => CATEGORY_LABELS.es[c] ?? c,
            results: (total) => `${total} producto${total === 1 ? "" : "s"} encontrado${total === 1 ? "" : "s"}`,
        ...
        en: {
            ...
            activeFiltersMaterial: (m) => m,
            activeFiltersCategory: (c) => CATEGORY_LABELS.en[c] ?? c,
            results: (total) => `${total} product${total === 1 ? "" : "s"} found`,
    ...
    return (<div className="min-h-screen py-8">
      <div className="max-w-site mx-auto px-4">
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-8">
          <form onSubmit={handleSearch} className="flex-1 flex gap-2">
            <div className="relative flex-1">
              <input type="text" value={search} onChange={e => setSearch(e?.target?.value ?? "")} placeholder={t.searchPlaceholder} className="w-full bg-bg-card rounded-lg pl-10 pr-4 py-2.5 text-sm outline-none focus:ring-1 focus:ring-cyan placeholder-zinc-500 border border-white/5"/>
            </div>
          ...
          <div className="flex gap-2">
            <button onClick={() => setFiltersOpen(!filtersOpen)} className="flex items-center gap-2 px-4 py-2.5 bg-bg-card border border-white/5 rounded-lg text-sm hover:bg-bg-hover transition">
            ...
            <select value={sort} onChange={e => { setSort(e?.target?.value ?? "newest"); setPage(1); }} className="bg-bg-card border border-white/5 rounded-lg px-3 py-2.5 text-sm outline-none focus:ring-1 focus:ring-cyan">
              {SORTS[language].map(s => (<option key={s.value} value={s.value}>{s.label}</option>))}
            </select>
          </div>
        </div>

        {/* Filters Panel */}
        {filtersOpen && (<motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} className="mb-8 bg-bg-card rounded-xl p-6 border border-white/5">
            <div className="flex items-center justify-between mb-4">
              <h3 className="font-semibold flex items-center gap-2">
                <Filter className="w-4 h-4"/> {t.filters}
              </h3>
              <button onClick={() => { setMaterial(""); setCategory(""); setPage(1); }} className="text-xs text-cyan hover:underline">
                {t.clearFilters}
              </button>
            </div>
            <div className="grid md:grid-cols-2 gap-6">
              <div>
                ...
                <div className="flex gap-2">
                  {["", "PLA", "PETG"].map(m => (<button key={m} onClick={() => { setMaterial(m); setPage(1); }} className={`px-4 py-2 rounded-lg text-sm font-medium transition ${material === m ? MATERIAL_ACTIVE_CLASSES[m] : "bg-white/5 text-zinc-400 border border-transparent hover:bg-white/10"}`}>{m || (language === "es" ? "Todos" : "All")}</button>))}
                </div>
              </div>
              <div>
                <label className="text-sm text-zinc-400 mb-2 block">{t.categoryLabel}</label>
                <div className="flex flex-wrap gap-2">
                  {CATEGORIES.map(c => (<button key={c} onClick={() => { setCategory(c === "Todos" ? "" : c); setPage(1); }} className={`px-3 py-1.5 rounded-lg text-sm transition ${(category || "Todos") === c ? "bg-cyan/20 text-cyan border border-cyan/30" : "bg-white/5 text-zinc-400 hover:bg-white/10"}`}>{CATEGORY_LABELS[language][c]}</button>))}
                </div>
              </div>
            </div>
          </motion.div>)}
        ...
        {/* Active Filters */}
        {(material || category) && (<div className="flex flex-wrap gap-2 mb-6">
            {material && (<span className="flex items-center gap-1 px-3 py-1 rounded-full text-xs bg-white/10">
                {t.activeFiltersMaterial(material)} <button onClick={() => setMaterial("")}><XIcon className="w-3 h-3"/></button>
              </span>)}
            {category && (<span className="flex items-center gap-1 px-3 py-1 rounded-full text-xs bg-white/10">
        ...
        {loading && (<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {(['sk-0', 'sk-1', 'sk-2', 'sk-3', 'sk-4', 'sk-5']).map((sk) => (<div key={sk} className="bg-bg-card rounded-lg overflow-hidden animate-pulse">
        ...
        {!loading && (products?.length ?? 0) === 0 && (<div className="text-center py-20">
            <p className="text-zinc-400 mb-4">{t.noProducts}</p>
            <button onClick={() => { setSearch(""); setMaterial(""); setCategory(""); setPage(1); }} className="px-4 py-2 bg-cyan/10 text-cyan rounded-lg text-sm hover:bg-cyan/20 transition">
              {t.clearFiltersButton}
            </button>
          </div>)}
        {!loading && (products?.length ?? 0) > 0 && (<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {(products ?? []).map((p, i) => (<motion.div key={p?.id ?? i} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}>
                <Link href={`/product/${p?.id}`} className="block group">
                  <div className="bg-bg-card rounded-lg overflow-hidden card-shadow card-shadow-hover transition-all duration-300 border border-white/5">
                    ...
                    <div className="p-4">
                      ...
                      {/* Color swatches */}
                      {(p?.colors?.length ?? 0) > 0 && (<div className="flex items-center gap-1 mb-2">
                          {(p?.colors ?? []).slice(0, 3).map((color) => (<span key={color} className="w-4 h-4 rounded-full border border-white/20" style={{ backgroundColor: getColorHex(color) }} title={color}/>))}
                          {(p?.colors?.length ?? 0) > 3 && (<span className="text-xs text-zinc-500">
                              +{(p?.colors?.length ?? 0) - 3}
                            </span>)}
                        </div>)}
                    ...
                    const price = calculatePriceFromDimensions(p?.defaultDimX ?? 0, p?.defaultDimY ?? 0, p?.defaultDimZ ?? 0, p?.printTimeMinutes ?? 60, mat, {
                    ...
                    return (<span className="font-mono text-cyan text-2xl font-bold">
        ...
        {totalPages > 1 && (<div className="flex items-center justify-center gap-2 mt-10">
            <button onClick={() => setPage(Math.max(1, page - 1))} disabled={page <= 1} className="p-2 rounded-lg bg-bg-card border border-white/5 disabled:opacity-30 hover:bg-bg-hover transition" aria-label={t.prevPageAria}>
              <ChevronLeft className="w-4 h-4"/>
            </button>
            {Array.from({ length: totalPages }).map((_, i) => (<button key={i + 1} onClick={() => setPage(i + 1)} className={`w-9 h-9 rounded-lg text-sm font-medium transition ${page === i + 1 ? "bg-cyan text-black" : "bg-bg-card border border-white/5 hover:bg-bg-hover"}`}>{i + 1}</button>))}
            <button onClick={() => setPage(Math.min(totalPages, page + 1))} disabled={page >= totalPages} className="p-2 rounded-lg bg-bg-card border border-white/5 disabled:opacity-30 hover:bg-bg-hover transition" aria-label={t.nextPageAria}>
