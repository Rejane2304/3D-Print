...
const COLOR_MAP = {
  ...
  Nogal: "#5D432C",
  "Madera Clara": "#DEB887",
  "Roble Natural": "#C4A35A",
  Marmol: "#E8E8E8",
  "Blanco Mármol": "#FAFAFA",
  "Pastel Mix": "#FFB6C1",
  Multicolor: "#FF6B6B",
  "Dorado Antiguo": "#CFB53B",
  "RGB (con LEDs)": "#FF00FF",
  "Azul Transparente": "#ADD8E6",
  "Rosa Transparente": "#FFB6C1",
  "Negro/Dorado": "#1a1a1a",
  "Gris/Rojo": "#6B7280",
  "Transparente/Naranja": "#FFA500",
  "Negro/Rojo": "#1a1a1a",
  "Gris Metálico": "#71797E",
  "Azul Océano": "#0077BE",
  Madera: "#966F33",
  "Blanco (Obligatorio)": "#FFFFFF",
};
...
export function CatalogClient() {
  ...
  const t = {
    es: {
      title: (
        <>
          Catálogo de <span className="text-gradient-cyan">Productos</span>
        </>
      ),
      subtitle:
        "Explora nuestra colección de piezas impresas en 3D y personaliza cada una a tu medida.",
      ...
      <div className="max-w-site mx-auto px-4">
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-8">
          <form onSubmit={handleSearch} className="flex-1 flex gap-2">
            ...
            <button
              type="submit"
              className="px-4 py-2.5 bg-cyan text-black rounded-lg text-sm font-medium hover:bg-cyan-dim transition"
            >
              {t.searchButton}
            </button>
          </form>
          <div className="flex gap-2">
            ...
            <select
              value={sort}
              onChange={(e) => {
                setSort(e?.target?.value ?? "newest");
                setPage(1);
              }}
              className="bg-bg-card border border-white/5 rounded-lg px-3 py-2.5 text-sm outline-none focus:ring-1 focus:ring-cyan"
            >
        ...
        {filtersOpen && (
          <motion.div
            ...
            className="mb-8 bg-bg-card rounded-xl p-6 border border-white/5"
          >
                ...
                <div className="flex gap-2">
                  {["", "PLA", "PETG"].map((m) => (
                    <button
                      ...
                      className={`px-4 py-2 rounded-lg text-sm font-medium transition ${material === m ? MATERIAL_ACTIVE_CLASSES[m] : "bg-white/5 text-zinc-400 border border-transparent hover:bg-white/10"}`}
                    >
                      {m || (language === "es" ? "Todos" : "All")}
                    </button>
                  ))}
                </div>
              </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {CATEGORIES.map((c) => (
                    <button
                      key={c}
                      onClick={() => {
                        setCategory(c === "Todos" ? "" : c);
                        setPage(1);
                      }}
                      className={`px-3 py-1.5 rounded-lg text-sm transition ${(category || "Todos") === c ? "bg-cyan/20 text-cyan border border-cyan/30" : "bg-white/5 text-zinc-400 hover:bg-white/10"}`}
                    >
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            {material && (
              <span className="flex items-center gap-1 px-3 py-1 rounded-full text-xs bg-white/10">
                {t.activeFiltersMaterial(material)}{" "}
            ...
            {category && (
              <span className="flex items-center gap-1 px-3 py-1 rounded-full text-xs bg-white/10">
                {t.activeFiltersCategory(category)}{" "}
          ...
          <div className="flex items-center justify-center gap-2 mt-10">
            ...
            {Array.from({ length: totalPages }).map((_, i) => (
              <button
                key={i + 1}
                onClick={() => setPage(i + 1)}
                className={`w-9 h-9 rounded-lg text-sm font-medium transition ${page === i + 1 ? "bg-cyan text-black" : "bg-bg-card border border-white/5 hover:bg-bg-hover"}`}
              >
                {i + 1}
              </button>
            ))}
